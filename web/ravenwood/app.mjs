...
import {WORLD_KEY,PLAYER_KEY,newWorld,validateWorld,accessReason,move,reserve,openLocker,clock} from './engine.mjs';
...
function render(){
  ...
  app.innerHTML=`<div class="layout"><aside class="status" aria-label="角色與時間">
    ...
    <dl><dt>角色</dt><dd>${escape(state.player.name)}</dd><dt>住宿</dt><dd>${state.player.buildingId} 宿舍 · ${state.player.residentialAccess==='male_side'?'男':'女'}側<br>201 雙人房</dd><dt>目前位置</dt><dd>${escape(here.name)}</dd></dl>
    ...
    <section class="section"><h2>Actions <small>行動</small></h2><div class="actions">${here.items.filter(i=>['locker','own-locker'].includes(i.kind)).map(i=>`<button data-locker="${i.id}">${i.kind==='own-locker'?'打開自己的置物櫃':state.openedLockers.includes(i.id)?'查看已開啟的置物櫃':'嘗試開鎖 · 妙手判定'}</button>`).join('')||'<span class="muted">這裡目前沒有可執行的行動。</span>'}</div></section>
